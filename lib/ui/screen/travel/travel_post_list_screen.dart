...
class TravelPostListScreen extends StatefulWidget {
  const TravelPostListScreen({super.key});

  @override
  State<TravelPostListScreen> createState() => _TravelPostListScreenState();
}

class _TravelPostListScreenState extends State<TravelPostListScreen> {
  @override
    ...
    return BlocBuilder<TravelPostCubit, TravelPostState>(
      builder: (context, state) {
        ...
        if (state.loadingStatus == LoadingStatus.noData) {
          return const Center(
            child: Text("아직 작성된 게시글이 없습니다. 최초의 게시글을 작성해보세요!"),
          );
        }
